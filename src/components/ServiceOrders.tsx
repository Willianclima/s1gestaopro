...
export default function ServiceOrders({ 
  ...
  const [qrCodeModalOrder, setQrCodeModalOrder] = useState<ServiceOrder | null>(null);
  ...
  const handleDownloadQrPng = (orderId: string, title: string) => {
    const svgElement = document.getElementById(`qr-svg-modal-${orderId}`) || document.getElementById(`qr-svg-id`);
    ...
    img.onload = () => {
      ...
      if (ctx) {
        ...
        downloadLink.download = `QRCode_OS_${orderId}.png`;
        document.body.appendChild(downloadLink);
        downloadLink.click();
        document.body.removeChild(downloadLink);
        toastSuccess(`QR Code da OS #${orderId} baixado em formato PNG!`, "Download Concluído");
      }
    };
    img.src = "data:image/svg+xml;base64," + btoa(unescape(encodeURIComponent(svgData)));
  };

  const handlePrintQrCode = (os: ServiceOrder) => {
    const printWindow = window.open("", "_blank");
    if (printWindow) {
      const directUrl = `${window.location.origin}${window.location.pathname}?os=${os.id}`;
        ...
        <html>
          ...
          <body>
            <div class="card">
              ...
              <h2>OS #${os.id}</h2>
              <div class="title">${os.title}</div>
              
              <div class="qr-container" id="qr-container"></div>

              <div class="info-grid">
                <div><strong>Requisitante:</strong> ${clientName}</div>
                <div><strong>Categoria:</strong> ${os.category}</div>
                <div><strong>Data de Abertura:</strong> ${os.startDate}</div>
                <div><strong>Técnico:</strong> ${os.assignedTo || "Aguardando"}</div>
              </div>

              <div class="footer">Araçatuba Serviços • Leitura com Câmera Mobile</div>
            </div>
            <script>
              document.getElementById("qr-container").innerHTML = '<img src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=' + encodeURIComponent("${directUrl}") + '" width="200" height="200" alt="QR Code OS #${os.id}" />';
  ...
  return (
    <div className="space-y-6">
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2 text-[11px]">
                              ...
                              {/* Quick Status Move & PDF Export Menu */}
              ...
              <div className="bg-slate-50 border border-slate-200/60 rounded-2xl p-4 flex flex-col sm:flex-row items-center gap-4">
                ...
                <div className="text-left space-y-1.5 flex-1">
                  ...
                  <div className="pt-1 flex gap-2 flex-wrap">
                    ...
                    <button
                      type="button"
                      onClick={() => handleDownloadQrPng(selectedOrder.id, selectedOrder.title)}
                      className="bg-indigo-50 border border-indigo-200 hover:bg-indigo-100 text-indigo-800 font-bold text-[10px] py-1.5 px-3 rounded-lg transition-colors flex items-center gap-1 shadow-2xs cursor-pointer"
                    >
                    ...
                    <button
                      type="button"
                      onClick={() => handlePrintQrCode(selectedOrder)}
      ...
      {/* DEDICATED QR CODE GENERATOR MODAL */}
      {qrCodeModalOrder && (
          ...
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl border border-slate-100 overflow-hidden flex flex-col">
            ...
            <div className="bg-slate-900 text-white p-5 px-6 flex items-center justify-between border-b border-slate-800">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="font-extrabold text-base tracking-tight leading-none">Gerador de QR Code</h3>
                  <p className="text-xs text-slate-400 font-mono mt-1">OS #{qrCodeModalOrder.id}</p>
                </div>
              </div>
              <button
                onClick={() => setQrCodeModalOrder(null)}
            ...
            {/* Modal Body */}
            <div className="p-6 space-y-5 text-center flex flex-col items-center text-slate-700">
              {/* OS Overview summary */}
              <div className="w-full text-left bg-slate-50 border border-slate-200/80 p-3.5 rounded-2xl space-y-1 text-xs">
                <div className="flex justify-between items-start gap-2">
                  <h4 className="font-extrabold text-slate-900 text-sm truncate">{qrCodeModalOrder.title}</h4>
                  {getPriorityBadge(qrCodeModalOrder.priority)}
                </div>
                <p className="text-slate-500 font-semibold">
                  Requisitante: <span className="text-slate-800">{getClientName(qrCodeModalOrder.clientId)}</span>
                </p>
                <div className="flex items-center justify-between pt-1 border-t border-slate-200/60 text-[11px]">
                  <span className="text-slate-500 font-medium">Categoria: <strong className="text-slate-700">{qrCodeModalOrder.category}</strong></span>
                  <span className={`px-2 py-0.5 rounded-md font-bold text-[9.5px] uppercase ${
                    qrCodeModalOrder.status === "concluido" ? "bg-green-100 text-green-800" :
                    qrCodeModalOrder.status === "em_progresso" ? "bg-blue-100 text-blue-800" :
                    qrCodeModalOrder.status === "aguardando" ? "bg-amber-100 text-amber-800" :
                    qrCodeModalOrder.status === "cancelado" ? "bg-red-100 text-red-800" : "bg-slate-100 text-slate-700"
                  }`}>
                    {qrCodeModalOrder.status === "concluido" ? "Concluído" :
                     qrCodeModalOrder.status === "em_progresso" ? "Em Execução" :
                     qrCodeModalOrder.status === "aguardando" ? "Aguardando Material" :
                     qrCodeModalOrder.status === "aberto" ? "Aberto / Pendente" : "Cancelado"}
                  </span>
                </div>
              </div>

              {/* The QR Code Canvas / SVG Container */}
              <div className="bg-white p-4 rounded-2xl border-2 border-indigo-100 shadow-md flex flex-col items-center justify-center relative group">
                <QRCodeSVG 
                  id={`qr-svg-modal-${qrCodeModalOrder.id}`}
                  value={`${window.location.origin}${window.location.pathname}?os=${qrCodeModalOrder.id}`}
                  size={180}
                  level="H"
                  includeMargin={true}
                />
                <div className="mt-2 text-[10px] font-mono font-bold text-slate-400 uppercase tracking-wider">
                  Escanear para Acesso Direto
                </div>
              </div>

              {/* URL Deep link display with quick copy */}
              <div className="w-full space-y-1.5 text-left">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">
                  Link Direto de Redirecionamento (URL Query Param <code className="font-mono text-slate-600">?os={qrCodeModalOrder.id}</code>)
                </label>
                <div className="flex gap-2">
                  <input
                    type="text"
                  ...
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(`${window.location.origin}${window.location.pathname}?os=${qrCodeModalOrder.id}`);
                      toastSuccess("Link de acesso copiado com sucesso!", "Redirecionamento Pronto");
                    }}
                    className="bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-xs px-3 py-2 rounded-xl transition-all cursor-pointer shadow-2xs"
                  >
                    Copiar
                  </button>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2.5 w-full pt-1">
                <button
                  onClick={() => handleDownloadQrPng(qrCodeModalOrder.id, qrCodeModalOrder.title)}
                  className="bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-xs py-2.5 px-3 rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
                >
                  <Download className="w-4 h-4 text-indigo-200" />
                  Baixar PNG
                </button>
                ...
                <button
                  onClick={() => handlePrintQrCode(qrCodeModalOrder)}
                  className="bg-slate-100 hover:bg-slate-200 text-slate-800 font-extrabold text-xs py-2.5 px-3 rounded-xl transition-all flex items-center justify-center gap-1.5 border border-slate-200 cursor-pointer"
                >
              ...
              {/* Open full OS detail view */}
              <button
                onClick={() => {
                  const osToOpen = qrCodeModalOrder;
                  setQrCodeModalOrder(null);
                  setSelectedOrder(osToOpen);
                }}
                className="w-full bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold text-xs py-2.5 rounded-xl border border-slate-200 transition-colors flex items-center justify-center gap-1.5 cursor-pointer mt-1"
              >
                <Eye className="w-4 h-4 text-slate-500" />
                Abrir Ficha Completa da OS #{qrCodeModalOrder.id}
              </button>

              {/* Scanning Explanation note */}
