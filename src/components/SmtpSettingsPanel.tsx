...
import { 
  ...
  Calendar, Clock, Trash2, History, RotateCcw, Play, Tag, Plus, Edit3, Layers, Search
} from "lucide-react";
import { SmtpSettings, WhatsappSettings, Almoxarifado, CurrentUser, Client, ServiceOrder, AppNotification, ServiceCategory } from "../types";
...
interface SmtpSettingsPanelProps {
  ...
  initialSubTab?: "smtp" | "whatsapp" | "backup" | "permissions" | "almoxarifados" | "categorias" | "push_diagnostic" | "material_diagnostic";
  clients?: Client[];
  orders?: ServiceOrder[];
...
export default function SmtpSettingsPanel({ 
  ...
  // Tab control
  const [activeSubTab, setActiveSubTab] = useState<"smtp" | "whatsapp" | "backup" | "permissions" | "almoxarifados" | "categorias" | "push_diagnostic" | "material_diagnostic">(
  ...
  const handleOpenNewCategoryModal = () => {
    if (!isAdminUser) {
      onNotifyTest("Acesso Restrito", "Apenas usuários Administrativos têm permissão para cadastrar novas categorias.", "error");
      return;
    }
  ...
  const handleOpenEditCategoryModal = (cat: ServiceCategory) => {
    if (!isAdminUser) {
      onNotifyTest("Acesso Restrito", "Apenas usuários Administrativos têm permissão para editar categorias.", "error");
      return;
    }
    setEditingCategory(cat);
    setCatNameInput(cat.name);
    setCatColorInput(cat.color || "blue");
    setIsCategoryModalOpen(true);
  };
  ...
  const handleSaveCategorySubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!isAdminUser) {
      onNotifyTest("Acesso Restrito", "Apenas usuários Administrativos têm permissão para alterar categorias.", "error");
      return;
    }
    if (!catNameInput.trim()) {
      onNotifyTest("Campo Obrigatório", "Informe o nome da categoria de serviço.", "error");
      return;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {activeSubTab === "categorias" && (
          <div className="lg:col-span-3 bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xs p-6 space-y-6 animate-fade-in text-left">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-100 dark:border-slate-800">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-base font-extrabold text-slate-900 dark:text-slate-100">Categorias de Serviço</h3>
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      Cadastre, edite e gerencie as áreas técnicas utilizadas nos chamados e no perfil dos técnicos.
                    </p>
                  </div>
                </div>
              </div>
              ...
              {isAdminUser ? (
                <button
                  ...
                  className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-md hover:shadow-indigo-200 flex items-center gap-2 cursor-pointer self-start md:self-auto"
                >
                  <Plus className="w-4 h-4" />
                  Nova Categoria
                </button>
              ) : (
                <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl text-xs font-bold border border-slate-200 dark:border-slate-700">
                  <Lock className="w-3.5 h-3.5 text-slate-400" />
                  <span>Apenas Leitura (Requer Usuário Administrativo)</span>
                </div>
              )}
            </div>
            ...
            {!isAdminUser && (
              <div className="p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40 rounded-2xl flex items-start gap-3 text-amber-800 dark:text-amber-300 text-xs">
                <ShieldAlert className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
                <div>
                  <span className="font-bold block">Acesso Restrito a Usuários Administrativos</span>
                  A inserção, alteração e exclusão de Categorias do Serviço é uma funcionalidade restrita exclusivamente a usuários com perfil de **Administrador** ou **Gestor**.
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="relative flex-1 max-w-xs">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs text-slate-800 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none"
                />
              </div>
              <span className="text-xs font-extrabold text-slate-500 dark:text-slate-400">
                Total: <strong className="text-slate-800 dark:text-slate-200">{categories.length}</strong> Categoria(s) cadastrada(s)
              </span>
            </div>

            {/* Categories Table / Cards Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {categories
                .filter(c => c.name.toLowerCase().includes(catSearchTerm.toLowerCase()))
                .map((cat) => {
                  const linkedOsCount = orders.filter(o => o.category === cat.name).length;
                  const colorBadgeClasses: Record<string, string> = {
                    ...
                    orange: "bg-orange-100 text-orange-800 border-orange-200 dark:bg-orange-950/60 dark:text-orange-300 dark:border-orange-800",
                    red: "bg-red-100 text-red-800 border-red-200 dark:bg-red-950/60 dark:text-red-300 dark:border-red-800",
                    ...
                    cyan: "bg-cyan-100 text-cyan-800 border-cyan-200 dark:bg-cyan-950/60 dark:text-cyan-300 dark:border-cyan-800",
                    amber: "bg-amber-100 text-amber-800 border-amber-200 dark:bg-amber-950/60 dark:text-amber-300 dark:border-amber-800",
                    indigo: "bg-indigo-100 text-indigo-800 border-indigo-200 dark:bg-indigo-950/60 dark:text-indigo-300 dark:border-indigo-800",
                    rose: "bg-rose-100 text-rose-800 border-rose-200 dark:bg-rose-950/60 dark:text-rose-300 dark:border-rose-800"
                  };
                  const colorClass = colorBadgeClasses[cat.color || "blue"] || colorBadgeClasses.blue;

                  return (
                    <div 
                      key={cat.id} 
                      className="p-4 bg-slate-50/70 dark:bg-slate-900/40 rounded-2xl border border-slate-200/80 dark:border-slate-800 flex flex-col justify-between gap-3 hover:border-slate-300 dark:hover:border-slate-700 transition-all"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex items-center gap-2">
                          <span className={`px-2.5 py-1 rounded-lg text-xs font-black uppercase tracking-wide border ${colorClass}`}>
                            {cat.name}
                          </span>
                        </div>
                      ...
                      <div className="text-xs text-slate-500 dark:text-slate-400 space-y-1">
                        <p className="flex items-center justify-between">
                          <span>Ordens de Serviço vinculadas:</span>
                          <strong className="text-slate-800 dark:text-slate-200 font-bold">{linkedOsCount} OS</strong>
                        </p>
                      </div>
                      ...
                      {isAdminUser && (
                        <div className="pt-2 border-t border-slate-200/60 dark:border-slate-800 flex items-center justify-end gap-2">
                          <button
                            ...
                            className="px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer"
                          >
                          ...
                          <button
                            ...
                            title={linkedOsCount > 0 ? "A categoria possui OS vinculadas e não pode ser excluída" : "Excluir Categoria"}
                            className={`px-3 py-1.5 border rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${
                              linkedOsCount > 0
                                ? "bg-slate-100 dark:bg-slate-850 text-slate-400 border-slate-200 dark:border-slate-800 cursor-not-allowed opacity-60"
                                : "bg-white dark:bg-slate-800 border-red-200 dark:border-red-900/50 hover:bg-red-50 dark:hover:bg-red-950/30 text-red-600 dark:text-red-400 cursor-pointer"
                            }`}
                          >
            ...
            {isCategoryModalOpen && (
              <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
                <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-md shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden flex flex-col">
                  ...
                  <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-extrabold text-sm uppercase tracking-wider">
                        {editingCategory ? "Editar Categoria de Serviço" : "Inserir Nova Categoria"}
                      </h3>
                    </div>
                    <button
                      type="button"
                  ...
                  <form onSubmit={handleSaveCategorySubmit} className="p-6 space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full px-3.5 py-2.5 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 rounded-xl text-xs font-bold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 outline-none"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        className="w-full px-3.5 py-2.5 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 rounded-xl text-xs font-bold text-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 outline-none"
                      >
                        <option value="blue">🟦 Azul (Suporte & TI)</option>
                        <option value="orange">🟧 Laranja (Eletrodomésticos / Ar)</option>
                        <option value="red">🟥 Vermelho (Mecânica / Urgências)</option>
                        <option value="purple">🟪 Roxo (Design & Planejamento)</option>
                        <option value="green">🟩 Verde (Elétrica Geral)</option>
                        <option value="cyan">🩵 Ciano (Hidráulica & Encanamento)</option>
                        <option value="amber">🟧 Âmbar (Fundações & Alvenaria)</option>
                        <option value="indigo">🟦 Anil (Construções Gerais)</option>
                        <option value="rose">🩷 Rosa (Pintura & Acabamento)</option>
                      </select>
                    </div>
                    ...
                    <div className="pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-2">
                      ...
                      <button
                        type="submit"
                        className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-md hover:shadow-indigo-200 flex items-center gap-1.5 cursor-pointer"
                      >
