...
export default function Clients({ clients, orders, onAddClient, onUpdateClient, onDeleteClient }: ClientsProps) {
  ...
  const [userType, setUserType] = useState<"requisitante" | "gestor">("requisitante");
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400 font-mono mt-0.5">{client.document || "Sem CPF / CNPJ"}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-1.5 opacity-80 group-hover:opacity-100 transition-opacity">
                      ...
                      <button 
                        onClick={() => {
                          if(confirm(`Tem certeza que deseja remover este ${ut === "gestor" ? "gestor" : "requisitante"}? Se houver ordens de serviço vinculadas, elas ficarão sem requisitante associado.`)) {
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0">
              <div className="p-6 space-y-4 overflow-y-auto flex-1">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">CPF / CNPJ</label>
                    <input
                      type="text"
                      className="w-full text-sm border border-slate-200 rounded-xl px-3.5 py-2.5 focus:outline-none focus:ring-2 focus:ring-slate-500/10 focus:border-slate-800 bg-slate-50/50 transition-all font-medium text-slate-700"
